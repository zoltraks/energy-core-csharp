...
namespace SqlServerConnection
{
    class Program
    {
        ...
        private static void Go(string[] args)
        {
            ...
            string connectionString = @"Data Source=.;Initial Catalog=Test;Integrated Security=Yes;Connect Timeout=10;";
            ...
            string err;
            string result = connection.Scalar<string>("SELECT GETDATE()", out err);
            if (err.Length > 0)
            {
                Energy.Core.Tilde.WriteLine(ConsoleColor.Red, err);
            }
